...
import { no, cached, extend, camelize, hyphenate } from './util';
import { isIE, isEdge, isServerRendering } from './env';
...
function processSlotContent(el: ASTElement) {
  ...
  if (slotTarget) {
    ...
    if (el.tag !== 'template' && !el.slotScope) {
      addAttr(el, 'slot', slotTarget, getRawBindingAttr(el, 'slot') as any);
    }
  }
  ...
  for (i = 0, l = list.length; i < l; i++) {
    ...
    if (dirRE.test(name)) {
      ...
      if (process.env.VBIND_PROP_SHORTHAND && propBindRE.test(name)) {
        ((modifiers || (modifiers = {})) as any).prop = true;
      ...
      if (bindRE.test(name)) {
        ...
        if (
          ...
          addProp(el, name, value, list[i] as any, isDynamic);
        } else {
          addAttr(el, name, value, list[i] as any, isDynamic);
        }
      ...
      addAttr(el, name, JSON.stringify(value), list[i] as any);
      ...
      if (
        ...
        addProp(el, name, 'true', list[i] as any);
      }
    }
  }
}
...
export function parse(
  ...
  function warnOnce(msg: string, range: Range) {
    if (!warned) {
      warned = true;
      warn(msg, range);
    }
...
function checkForAliasModel(el: ASTElement, value: string) {
  let _el = el;
  while (_el) {
