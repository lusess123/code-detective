import { ASTElement, ASTModifiers } from './type';
...
export function addProp(
  ...
  (el.props || (el.props = [])).push(
    rangeSetItem({ name, value, dynamic }, range as any),
  );
  el.plain = false;
}
...
export function addAttr(
  ...
  const attrs = dynamic
    ...
    : el.attrs || (el.attrs = []);
  attrs.push(rangeSetItem({ name, value, dynamic }, range as any));
  el.plain = false;
}
...
export function pluckModuleFunction<F extends Function>(
  modules: Array<Record<string, any>> | undefined,
  key: string,
): Array<F> {
